...
async def _probe_vulkan() -> Optional[GpuInfo]:
...
async def _probe_metal(whisper_cli: Optional[Path] = None) -> Optional[GpuInfo]:
...
async def detect_gpu(*, whisper_cli: Optional[Path] = None) -> GpuInfo:
