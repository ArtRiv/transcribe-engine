#!/usr/bin/env bash
# scripts/build_whisper_cpp_macos.sh
#
# Idempotent builder for whisper.cpp v1.8.4 with the Metal backend enabled.
# Produces $BUILD_DIR/build/bin/whisper-cli — consumed via --add-binary in
# scripts/build.sh (PyInstaller step).
#
# macOS variant: -DGGML_METAL=1 (NOT Vulkan). Requires Xcode Command Line Tools.
...
# Xcode CLT check (required for Metal build)
...
if [ -x "$WHISPER_CLI" ] && [ -d "$BUILD_DIR/.git" ]; then
  ...
  if [ "$HEAD_SHA" = "$TAG_SHA" ] && [ "$HEAD_SHA" != "none" ]; then
    echo "[build_whisper_cpp_macos.sh] already built at $PIN_TAG ($HEAD_SHA); skipping" >&2
    echo "$WHISPER_CLI"
    exit 0
  fi
fi
...
if [ ! -d "$BUILD_DIR/.git" ]; then
  echo "[build_whisper_cpp_macos.sh] cloning whisper.cpp into $BUILD_DIR" >&2
...
# ---------------------------------------------------------------------------
# 4. Configure + build with the Metal backend (-DGGML_METAL=1).
# ---------------------------------------------------------------------------
echo "[build_whisper_cpp_macos.sh] configuring cmake (-DGGML_METAL=1)" >&2
cmake -S "$BUILD_DIR" -B "$BUILD_DIR/build" \
  -DGGML_METAL=1 \
  -DCMAKE_BUILD_TYPE=Release

echo "[build_whisper_cpp_macos.sh] building (this can take 5-10 min on first run)" >&2
...
# Binary-content probe: verify GGML Metal symbol is present.
if ! grep -q "ggml-metal" "$WHISPER_CLI" 2>/dev/null; then
  echo "WARN: 'ggml-metal' symbol not visible via grep; deferring Metal check to runtime probe" >&2
fi

BUILD_SHA="$(git -C "$BUILD_DIR" rev-parse HEAD)"
echo "[build_whisper_cpp_macos.sh] whisper-cli built: $WHISPER_CLI ($PIN_TAG @ $BUILD_SHA)" >&2
